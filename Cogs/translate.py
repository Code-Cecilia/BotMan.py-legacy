...
class Translate(commands.Cog, description='A set of commands that uses the google translate API'):
    ...
    async def translate(self, ctx, *, text):
        ...
        if str(src_lang) not in self.lang_list:
            has_src = True
        else:
            has_src = False
        if dest_lang not in self.lang_list:
            return print(f'Could not find language {dest_lang}.')

        split_text.pop(-1)  # remove destination lanf
        if not has_src:  # remove source lang if exists
            split_text.pop(-1)
